...
자바 진영에서 가장 많이 사용되는 로깅 프레임워크는 [Logback]이다. Logback이란 log4j 이후에 출시된 로깅 프레임워크로서 [slf4j]를 기반으로 구현됐으며, 과거에 사용되던 log4j에 비해 월등한 성능을 자랑한다. 또한 스프링 부트의 spring-boot-starter-web 라이브러리 내부에 내장돼 있어 별도의 의존성을 추가하지 않아도 사용할 수 있다.
