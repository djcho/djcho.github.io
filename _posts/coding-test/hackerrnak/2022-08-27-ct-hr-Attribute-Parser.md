...
일단 문제의 핵심은 Hrml 이라는 소스코드를 파싱 하는 것이기 때문에 `HrmlParser`를 정의하였다. `HrmlParser`는 주어진 소스코드를 입력으로 받아 인스턴스를 생성하고 `parser()` 메서드로 내부 자료구조에 파싱된 데이터를 인스턴스화시키고 메서드로 정해진 규칙에 따라 입력되는 QrueyString을 `query()`  메서드를 통해 결과를 출력하는 객체이다.

<br>

이 객체에는 위 메서드 외에 태그 이름을 구하는 `getTagName()`메서드와 어트리뷰트의 목록을 파싱하는 `getAttributes()` , Closer 인지를 확인하는`Closer()` 존재한다. 메인 파싱 로직은 최초 입력 받은 소스코드를 한줄씩 읽으면서 새로 만나는 태그들의 객체를 생성하고 이 객체의 부모와 자식 관계를 연결한다.

<br>

`query()` 메서드에서는 "`~`" 로 Split 하여 태그의 위치를 "`.`"으로 표현하는 위치 부분과 쿼리 대상 어트리뷰트의 이름을 나누고, 이 데이터를 가지고 내부 자료구조들을 탐색한다.
