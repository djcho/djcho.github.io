...
`WPF 클래스 라이브러리` 혹은 `WPF 앱(.NET Core)`  프로젝트로 생성하지 않도록 주의 한다. {: .notice--warning}
...
CLR 클래스 라이브러리(.NET) 프로젝트와 헷갈리지 않게 주의 한다. 이 프로젝트로 만들 경우 Wpf 클래스 라이브러리를 참조 추가할 수 없다.{: .notice--warning}
